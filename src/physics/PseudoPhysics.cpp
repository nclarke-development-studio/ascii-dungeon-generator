...
void PhysicsSystem::Generate(Floor<int> *floor, int fill, bool dense, int minS, int maxS, int numSteps)
{
	...
	vector<UndirectedEdge<RoomNode>> links;

	auto tmpLinks = mst.GetEdges();
	auto rooms = mst.GetNodes();

	// remove links with a weight of 0
	std::copy_if(tmpLinks.begin(), tmpLinks.end(), std::back_inserter(links), [](UndirectedEdge<RoomNode> i)
				 { return i.weight > 0; });
	...
	AddUndirectedEdgesToFloor(*floor, links);
...
void PhysicsSystem::AddCycles(Graph<RoomNode> &MST)
{
	for (int i = 0; i < nodes.size() - 1; i++)
	{
		...
		for (auto &node : sur)
		{
			...
			{
				MST.AddUndirectedEdge(MST.GetNode(nodes[i]), MST.GetNode(node), distance);
			}
		}
	}
}
...
void PhysicsSystem::AddDenseCycles(Graph<RoomNode> &MST)
{
	...
	for (int i = 0; i < nodes.size() - 1; i++)
	{
		for (int j = i + 1; j < nodes.size(); j++)
		{
			...
			if (distance > nodes.size() / 2)
			{
				MST.AddUndirectedEdge(nodes[i], nodes[j], distance);
			}
		}
	}
}
...
void PhysicsSystem::AddUndirectedEdgesToFloor(Floor<int>& f, vector<UndirectedEdge<RoomNode>> &edges)
{
	for (auto &edge : edges)
	{
		f.ConnectRegions(edge.u->data->GetRoom(),
							  edge.v->data->GetRoom(), true, _fill);
	}
}
